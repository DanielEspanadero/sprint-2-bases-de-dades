...
SELECT a.nombre, ce.anyo_inicio, ce.anyo_fin FROM persona p JOIN alumno_se_matricula_asignatura ama ON p.id = ama.id_alumno JOIN curso_escolar ce ON ce.id = ama.id_curso_escolar JOIN asignatura a ON a.id = ama.id_asignatura WHERE p.nif = "26902806M";
...
SELECT DISTINCT p.* FROM persona p JOIN alumno_se_matricula_asignatura ama ON p.id = ama.id_alumno JOIN curso_escolar c ON c.id = ama.id_curso_escolar WHERE anyo_fin = 2019 AND anyo_inicio = 2018;
...
SELECT d.nombre as dept, p.apellido1, p.apellido2, p.nombre FROM persona p LEFT JOIN profesor pro ON p.id = pro.id_profesor LEFT JOIN departamento d ON pro.id_departamento = d.id WHERE p.tipo = "profesor" ORDER BY d.nombre, p.apellido1, p.apellido2, p.nombre;
--2º- Retorna un llistat amb els professors que no estan associats a un departament.
SELECT p.* FROM persona p LEFT JOIN profesor pro ON p.id = pro.id_profesor WHERE p.tipo = "profesor" AND pro.id_profesor IS NULL;
--3º- Retorna un llistat amb els departaments que no tenen professors associats.
SELECT d.* FROM departamento d LEFT JOIN profesor pro ON d.id = pro.id_departamento WHERE pro.id_departamento  IS NULL;
--4º- Retorna un llistat amb els professors que no imparteixen cap assignatura.
SELECT p.* FROM persona p JOIN profesor pro ON p.id = pro.id_profesor LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor WHERE a.id IS NULL;
...
SELECT d.* FROM alumno_se_matricula_asignatura ama LEFT JOIN asignatura a ON ama.id_asignatura = a.id LEFT JOIN profesor pro ON a.id_profesor = pro.id_profesor RIGHT JOIN departamento d ON pro.id_departamento = d.id WHERE ama.id_curso_escolar IS NULL; 
...
SELECT d.nombre AS departament, COUNT(*) AS num_professors FROM departamento d JOIN profesor pro ON d.id = pro.id_departamento GROUP BY d.id ORDER BY num_professors;
...
SELECT d.nombre AS departament, COUNT(pro.id_profesor) AS num_professors FROM departamento d LEFT JOIN profesor pro ON d.id = pro.id_departamento GROUP BY d.id;
...
SELECT p.* FROM profesor pro LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor JOIN persona p ON pro.id_profesor = p.id WHERE a.id IS NULL;